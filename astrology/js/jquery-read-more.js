...
(function($) {
  ...
  $.fn.readMore = function(options) {
    ...
    var maxLines = parseInt(options.lines),
      readMoreLabel = options.readMoreLabel || "Читать дальше",
      readLessLabel = options.readLessLabel || "Скрыть",
    ...
    $(this).each(function (_j, element) {
      ...
      if(overflow) {
        $readMore.text(readMoreLabel).css('display', 'inline-block');
        ...
        function callback() {
          if($(element).data().expanded) {
            $fullDiv.animate({height: minH + 'px'}, 400, function() {
            ...
            $fullDiv.css('text-align', 'left');
            $fullDiv.css('height', minH + 'px')
            $fullDiv.animate({height: realH + 'px'}, 400);
